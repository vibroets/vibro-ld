...
export const restoreVideosFromSupabase = async () => {
  try {
    const { data: videosData, error } = await supabase.from('videos').select('*');
    if (error) throw error;
    ...
    if (videosData && videosData.length > 0) {
      ...
      console.log('No videos found in Supabase, keeping current localStorage data');
      return { success: true, count: 0, message: 'No videos in Supabase' };
    }
  } catch (error) {
...
// Restore all data from Supabase (users, videos, quizzes, training schedules)
export const restoreAllDataFromSupabase = async () => {
  const results = {
    users: 0,
    videos: 0,
    quizzes: 0,
  ...
  try {
    ...
    if (schedulesData && schedulesData.length > 0) {
      ...
      results.trainingSchedules = schedules.length;
    }

    console.log('Restore results:', results);
    return { success: true, results };
  } catch (error) {
    console.error('Restore all data error:', error);
    throw error;
  }
};
